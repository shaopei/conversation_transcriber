...
transcript_path = os.path.join(basepath, f"{new_base}.raw_transcript.txt")
good_transcript_path = os.path.join(basepath, f"{new_base}.WM.transcript.txt")
summary_path = os.path.join(basepath, f"{new_base}.gpt4o.summary.txt")
...
# 9. Convert MOV to 480p MOVMP4 (ffmpeg required)
base, ext = os.path.splitext(input_mov)
output_mov_480p = base + '_480p' + ext  # 保持副檔名.mov，只在結尾加 _480p

if not os.path.exists(output_mov_480p):
    print(f"Converting {input_mov} to {output_mov_480p} ...")
    cmd = [
        'ffmpeg',
        '-i', input_mov,
        '-vf', 'scale=640:480',
    ...
    print(f"Converted to: {output_mov_480p}")
else:
    print(f"480p MOV file already exists: {output_mov_480p}")

# 11. Rename MOV
new_input_path = os.path.join(basepath, f"{new_base}{ext}")
...
# 12. 將 480p 檔案重新命名
new_480p_path = os.path.join(basepath, f"{new_base}_480p{ext}")
if os.path.exists(new_480p_path):
    print(f"Warning: {new_480p_path} already exists. Skipping renaming 480p file.")
else:
    os.rename(output_mov_480p, new_480p_path)
    print(f"已將 480p 影片重新命名為: {new_480p_path}")